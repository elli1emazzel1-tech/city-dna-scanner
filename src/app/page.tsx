...
export default function Home() {
  ...
  const [report, setReport] = useState<any>({
    ...
    overallScore: 74,
    overallStatus: "Good",
    summary: "Your city is making progress in several areas, but faces environmental challenges.",
    reportDate: "Aug 11, 2026 • 11:15 PM",
    metrics: {
    ...
    diagnosis: {
      mainText: "Your city has shown improvement in air and water quality over the past decade. Increasing climate risks and green space expansion remain key priorities.",
      bullets: [
        "Air quality score is currently stable at 82/100",
        "Heat stress mitigation needed in dense urban zones",
        "Green canopy coverage is below optimal target levels",
        "Waste recycling initiatives require infrastructure upgrades"
      ]
    },
    ...
    try {
      ...
      if (!data.error) {
        setReport({
          ...
          country: data.country || "Monitored Region",
          ...
          recommendations: data.recommendations
            ...
            : [
                { id: 1, title: "Expand urban forest canopy along major transit lines." },
                { id: 2, title: "Upgrade solid waste sorting infrastructure." }
              ]
        });
    ...
    <div className="min-h-screen bg-[#060913] text-slate-100 flex font-sans selection:bg-emerald-500 selection:text-slate-950">
      ...
      <aside className="w-64 border-r border-slate-800/60 p-6 flex flex-col justify-between hidden lg:flex bg-[#080D1A] shrink-0">
        <div className="space-y-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400 font-medium">AI Planet Health Scanner</p>
            </div>
          </div>
        ...
        <div className="space-y-4">
          <div className="relative w-full h-32 rounded-full overflow-hidden flex items-center justify-center opacity-80">
            <Globe className="w-28 h-28 text-cyan-500/20 animate-pulse" />
          </div>
          <div className="p-3 rounded-xl bg-[#0C1222] border border-slate-800/80 text-[11px] text-slate-400">
            <p className="font-semibold text-slate-200 mb-0.5">AI for a Better Planet</p>
            <p className="text-[10px] text-slate-500">Empowering decisions for a sustainable tomorrow.</p>
          </div>
        </div>
      </aside>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              AI analyzes environmental data to generate a complete health profile.
            </p>
          </div>
          ...
          <form onSubmit={handleScan} className="flex items-center gap-2">
            ...
            <button
              type="submit"
              disabled={loading}
              className="px-4 py-2 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold rounded-lg text-xs flex items-center gap-1.5 transition shrink-0"
            >
...
function MetricCard({ icon, label, score, trend, sparkline }: any) {
  const data = (sparkline || [50, 60, 70]).map((val: number, i: number) => ({ val, i }));
...
function DashboardView({ report, isSaved, handleSaveReport, setActiveTab }: any) {
  return (
    <div className="space-y-6">
      <div className="relative rounded-2xl overflow-hidden border border-slate-800/80 bg-[#0C1222] min-h-[190px] flex items-center p-6 lg:p-8">
        <div
          className="absolute inset-0 bg-cover bg-center opacity-20 filter brightness-75 contrast-125"
          style={{
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3.5">
          <MetricCard icon={<Wind className="w-4 h-4 text-sky-400" />} label="Air" score={report.metrics.air.score} trend={report.metrics.air.trend} sparkline={report.metrics.air.sparkline} />
          <MetricCard icon={<Droplets className="w-4 h-4 text-blue-400" />} label="Water" score={report.metrics.water.score} trend={report.metrics.water.trend} sparkline={report.metrics.water.sparkline} />
          <MetricCard icon={<Trees className="w-4 h-4 text-emerald-400" />} label="Green Space" score={report.metrics.green.score} trend={report.metrics.green.trend} sparkline={report.metrics.green.sparkline} />
          <MetricCard icon={<Thermometer className="w-4 h-4 text-amber-500" />} label="Climate Risk" score={report.metrics.climate.score} trend={report.metrics.climate.trend} sparkline={report.metrics.climate.sparkline} />
          <MetricCard icon={<Recycle className="w-4 h-4 text-yellow-500" />} label="Waste" score={report.metrics.waste.score} trend={report.metrics.waste.trend} sparkline={report.metrics.waste.sparkline} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="p-5 rounded-2xl bg-[#0C1222] border border-slate-800/80 flex flex-col justify-between space-y-4">
          <div>
            <h5 className="text-xs font-bold text-white mb-3 flex items-center gap-1.5">
              <Sparkles className="w-3.5 h-3.5 text-emerald-400" /> AI Diagnosis
            </h5>
            <p className="text-xs text-slate-300 leading-relaxed mb-4">{report.diagnosis.mainText}</p>
        ...
        <div className="p-5 rounded-2xl bg-[#0C1222] border border-slate-800/80 flex flex-col justify-between space-y-3">
          <div>
            <h5 className="text-xs font-bold text-white mb-3 flex items-center gap-1.5">
              <Clock className="w-3.5 h-3.5 text-cyan-400" /> City Health Timeline
            </h5>
        ...
        <div className="p-5 rounded-2xl bg-[#0C1222] border border-slate-800/80 flex flex-col justify-between space-y-4">
          <div>
            <h5 className="text-xs font-bold text-white mb-3 flex items-center gap-1.5">
              <Lightbulb className="w-3.5 h-3.5 text-amber-400" /> Top Recommendations
            </h5>
...
function TimelineView({ report }: any) {
  return (
    <div className="p-6 rounded-2xl bg-[#0C1222] border border-slate-800/80 space-y-6">
      <div>
        <h3 className="text-xl font-bold text-white flex items-center gap-2">
          <Clock className="w-5 h-5 text-cyan-400" /> Multi-Year Trajectory
        </h3>
        <p className="text-xs text-slate-400 mt-1">Historical score evolution for {report.cityName}.</p>
      </div>
  ...
  return (
    <div className="p-6 rounded-2xl bg-[#0C1222] border border-slate-800/80 space-y-5">
      <div>
        ...
        <p className="text-xs text-slate-400 mt-1">Targeted interventions for {report.cityName}.</p>
      </div>
...
function SavedView({ savedReports }: any) {
  return (
    <div className="p-6 rounded-2xl bg-[#0C1222] border border-slate-800/80 space-y-5">
      <h3 className="text-xl font-bold text-white flex items-center gap-2">
        <Bookmark className="w-5 h-5 text-emerald-400" /> Saved Reports ({savedReports.length})
      </h3>

      {savedReports.length === 0 ? (
        <div className="p-8 text-center bg-slate-900/30 rounded-xl border border-slate-800/50">
          <p className="text-xs text-slate-400">No saved reports yet. Scan a city and click "Save Report".</p>
        </div>
      ) : (
...
function AboutView() {
  return (
    <div className="p-6 rounded-2xl bg-[#0C1222] border border-slate-800/80 space-y-4 text-xs text-slate-300">
      <h3 className="text-xl font-bold text-white flex items-center gap-2">
        <Info className="w-5 h-5 text-cyan-400" /> About DNA of a City
      </h3>
